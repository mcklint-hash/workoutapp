...
function renderStats(){
 ...
 weeklyStats.innerHTML=weeks.length?`<div class="weekList">${weeks.map(w=>{const focus=Object.entries(w.muscles).sort((a,b)=>b[1]-a[1])[0]?.[0]||"–",width=Math.max(4,w.workouts/maxWeek*100);return`<article class="weekCard"><div class="weekTop"><div><b>${weekLabel(w.date)}</b><span>${w.workouts} pass · ${w.sets} set</span></div><strong>${w.workouts} ${w.workouts===1?"pass":"pass"}</strong></div><div class="barTrack"><div class="barFill" style="width:${width}%"></div></div><div class="statsSub">Mest tränade muskelgrupp: ${escapeHtml(focus)}</div></article>`;}).join("")}</div>`:'<div class="statsEmpty">Ingen veckostatistik för vald period.</div>';
